...
function readTextFile(file)
{
    ...
    rawFile.onreadystatechange = function ()
    {
        if(rawFile.readyState === 4)
        {
            ...
            {
                console.log(JSON.stringify(Object.keys(rawFile)))
...
var recordingDIV = document.querySelector('.recordrtc');
            ...
            document.querySelector('#start-recording-btn').onclick = function() {
            ...
            function captureAudioPlusVideo(config) {
                console.log("capture")
                captureUserMedia({'video': {facingMode: 'user'}, audio: true}, function(audioVideoStream) {
                    console.log(JSON.stringify(audioVideoStream))
                    let localUrl =Array.isArray(audioVideoStream)? audioVideoStream[0].fullPath : audioVideoStream.fullPath
                    const type = Array.isArray(audioVideoStream) ? audioVideoStream[0].type : audioVideoStream.type
                    const size =Array.isArray(audioVideoStream) ? audioVideoStream[0].size : audioVideoStream.size
                  ...
                  //  localUrl = localUrl.replace("file://", "")
                    ...
                    window.resolveLocalFileSystemURL(localUrl, entry=>{
                         entry.file(function (file) {
                                   ...
                                   fetch("http://52.201.120.80/laravel/api/proposiciones/upload",{
                                        method: "post",
                                        body: formData
            ...
            function captureUserMedia(mediaConstraints, successCallback, errorCallback) {
                console.log("here")
                navigator.device.capture.captureVideo(successCallback, errorCallback, {quality: 0.5 })
